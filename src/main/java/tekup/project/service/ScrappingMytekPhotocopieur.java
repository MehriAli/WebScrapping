...
@Service
@AllArgsConstructor
public class ScrappingMytekPhotocopieur {
	...
	public List<Dto> scrapelaptop() {
		String uri = "http://127.0.0.1:5000/mytekPhotocopieur";
		...
		for (int i = 0, j = 1; j < 13; i++, j++) {
			...
			Long myId = (long) j;
			laptop.add(new Dto(myId, mots[i], mots[i + 1], marque, mots[i + 2], price, disponibil,"MyTek"));
			i = i + 4;
		}
