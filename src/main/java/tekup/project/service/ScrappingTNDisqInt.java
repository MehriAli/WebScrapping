...
@Service
@AllArgsConstructor
public class ScrappingTNDisqInt {
	...
	public List<Dto> scrapelaptop() {
		String uri = "http://127.0.0.1:5000/tunisianetDisqueInt";
		...
		for (int i = 0, j = 1; j < 13; i++, j++) {
			//String str = mots[i + 2];
			String str = "jcnz ekcjnedc kedcnekc kzjdcnkec ljzdczldc,";
			String words[] = str.split(" ");
			String marque = words[0] + " " + words[1] + " " + words[2] + " " + words[3];
			//String str1 = mots[i + 3];
			String str1 = "ecd cede ceece ecec ece errerer";
			...
			Long myId = (long) j;
			laptop.add(new Dto(myId, mots[i], mots[i + 1], marque, mots[i + 2], price, disponibil,"Tunisia Net"));
			i = i + 4;
		}
